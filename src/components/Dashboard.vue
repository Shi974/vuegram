            ...
            <div class="col1">
                <div class="profile">
                    ...
                    <div class="create-post">
                        <p><i class="fas fa-pencil-alt"></i> Créer un post</p>
                        <form @submit.prevent>
                            <textarea v-model.trim="post.content"></textarea>
                            <button @click="createPost" :disabled="post.content == ''" class="button">Envoyer</button>
                        </form>
                    </div>
                </div>
            </div>
            <div class="col2">
                <transition name="fade">
                    <div v-if="hiddenPosts.length" @click="showNewPosts" class="hidden-posts">
                        <p>
                            Cliquer pour montrer <span class="new-posts">{{ hiddenPosts.length }}</span>
                            <span v-if="hiddenPosts.length > 1"> nouveaux posts</span><span v-else> nouveau post</span>
                        </p>
                    </div>
                </transition>
                <div v-if="posts.length">
                    <div v-for="post in posts" v-bind:key="post.id" class="post">
                        ...
                        <ul>
                            <li><a @click="openCommentModal(post)">commentaires {{ post.comments }}</a></li>
                            <li><a @click="likePost(post.id, post.likes)">likes {{ post.likes }}</a></li>
                            <li><a @click="viewPost(post)">voir le post</a></li>
                        </ul>
                    </div>
                </div>
                <div v-else>
                    <p class="no-results">Il n'y a rien en ligne ! <i class="far fa-grimace"></i></p>
                </div>
            </div>
        </section>
            ...
            <div v-if="showCommentModal" class="c-modal">
                <div class="c-container">
                    ...
                    <form @submit.prevent>
                        ...
                        <button @click="addComment" :disabled="comment.content == ''" class="button">commenter</button>
                    </form>
                </div>
            </div>
        </transition>
            ...
            <div v-if="showPostModal" class="p-modal">
                <div class="p-container">
                    ...
                    <div class="post">
                        ...
                        <ul>
                            <li><a>commentaires {{ fullPost.comments }}</a></li>
